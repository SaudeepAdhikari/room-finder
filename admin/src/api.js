...
// Fetch single room details (admin)
export async function fetchRoomByIdAdmin(id) {
	const res = await fetch(`${API_BASE}/rooms/${id}`, { credentials: 'include' });
	if (!res.ok) {
		const text = await res.text().catch(() => '');
		console.error('fetchRoomByIdAdmin error:', text);
